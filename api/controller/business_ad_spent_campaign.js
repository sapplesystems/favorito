...
exports.all_business_campaign = async function(req, res, next) {
    try {
        ...
        var sql = "SELECT id,`name`,keyword,cpc,total_budget, total_spent,impressions,clicks,status \n\
 ...
 */
 exports.create_campaign =async function(req, res, next) {
    try {
        var business_id = req.userdata.business_id;
        ...
        var status = await exports.getStatus(business_id,req.body.total_budget,req.body.status);
        if(status!=''){
            let mess=(status=='Stop')?'Your Wallet is low budget,this campaign can\'t run':'Ad campaign saved successfully';
...
exports.getStatus = function(business_id,total_budget,camp_status) {
    return new Promise(function(resolve, reject) {
        var sql = "SELECT (`free_credits`+`paid_credits`)AS total FROM business_ad_credits AS bac WHERE bac.business_id='"+business_id+"'";
        db.query(sql, function(err, total_credit) {
            var status=(total_credit[0].total>total_budget)? camp_status:'Stop';
            resolve(status);
        });
    });
}