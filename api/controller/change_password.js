...
exports.changePassword = function (req, res, next) {
  if (req.body.id == '' || req.body.id == 'undefined' || req.body.id == null) {
    ...
    return res.json({ status: 'error', message: 'Old password not found.' });
  } else if (req.body.new_password == '' || req.body.new_password == 'undefined' || req.body.new_password == null) {
    return res.json({ status: 'error', message: 'New password not found.' });
  } else if (req.body.confirm_password == '' || req.body.confirm_password == 'undefined' || req.body.confirm_password == null) {
    return res.json({ status: 'error', message: 'Confirm passwod not found.' });
  } else if (req.body.new_password != req.body.confirm_password) {
    return res.json({ status: 'error', message: 'New password and confirm password does not match.' });
  } else {
    ...
    db.query(sql, function (err, result) {
      if (err) {
        ...
        if (result.length === 0) {
          return res.status(500).json({ status: 'error', message: 'Incorrect business id' });
        }
        bcrypt.compare(req.body.old_password, result[0].password, function (err, enc_result) {
          if (err) {
            ...
            return res.status(500).json({ status: 'error', message: 'Old pssword does not match.', data: err });
          } else {
            bcrypt.hash(req.body.new_password, 10, function (err, hash) {
              ...
              db.query(uq, function (error, resp) {
                if (error) {
                  return res.status(500).json({ status: 'error', message: 'Password could not be changed.' });
                }
                return res.status(500).json({ status: 'error', message: 'Password changed successfully.' });
              });
            });
          }
        });
