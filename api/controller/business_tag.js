...
exports.addTag = function (req, res, next) {
    try {
        ...
        var sql = "INSERT INTO business_tags SET ?";
...
exports.listTag = function (req, res, next) {
    try {
        ...
        var sql = "SELECT id, `tag_name` FROM business_tags WHERE deleted_at IS NULL" + cond;
...
exports.updateTag = function (req, res, next) {
    try {
        ...
        var sql = "update business_tags set tag_name = '" + tag_name + "', updated_at=now() where id = '" + tag_id + "'";
        db.query(sql, function (err, result) {
            if (err) {
...
exports.deleteTag = function (req, res, next) {
    try {
        ...
        var sql = "update business_tags set deleted_at = now() where id = '" + tag_id + "'";
        db.query(sql, function (err, result) {
            if (err) {
                res.status(500).json({ status: 'error', message: 'Something went wrong.', data: err });
            }
