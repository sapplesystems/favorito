...
exports.register = async function(req, res, next) {
    try {
        ...
        } else if (req.body.fb_key == '' || req.body.fb_key == null) {
            return res.status(403).json({ status: 'error', message: 'Mobile number is not verified.Please verified again' });
        }
        ...
        if (resultCheckBusiness[0].count > 0) {
            return res.status(200).json({ status: 'error', message: 'Phone number already exist in business account' });
        }
            ...
            if (chkerr) {
                ...
                if (check[0].c === 0) {
                    var postval = {
                        ...
                        firebase_chat_id:req.body.fb_key
                    };
                    ...
                    db.query(sql, postval, function(err, result) {
                        ...
                        return res.status(200).json({ status: 'success', message: 'User Registered Successfully', id: result.insertId, phone: phone });
                    });
                } else {
                    return res.status(403).json({ status: 'error', message: 'Phone number already exist.' });
                }
            }
        });
    } catch (e) {
...
exports.registerEmail = async(req, res) => {
    if (!req.body.email) {
        return res.status(400).send({ status: 'error', message: 'Email is missing' })
    } else {
        var email = req.body.email
    }
    try {
    ...
    if (result_check_email[0].c > 0) {
        return res.status(403).json({ status: 'error', message: 'Username already exist.' });
    } else {
        try {
            update_email = `update users set email = '${email}' where id = '${req.userdata.id}'`
            result_update_email = await exports.run_query(update_email)
            var messageId = mail(email).catch(console.error);
            if (messageId) {
                return res.status(200).json({ status: 'success', message: 'Email registration successfull, mail sent.' });
            } else {
                return res.status(200).json({ status: 'success', message: 'Email registration successfull, mail sending failed.' });
            }
        } catch (error) {
            return res.status(500).json({ status: 'error', message: 'Something went wrong', error });
        }
...
exports.setPassword = async(req, res) => {
    ...
    bcrypt.hash(req.body.password, 10, async function(err, hash) {
        ...
        try {
            update_email = `update users set password = '${hash}' where id = '${req.userdata.id}'`
            result_update_email = await exports.run_query(update_email)
