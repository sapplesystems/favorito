...
exports.verifyOtp = async function(req, res, next) {
    try {
            let sql = "UPDATE business_master SET is_phone_verified='1' WHERE business_id='" + req.userdata.business_id + "'";
            var result = await exports.run_query(sql);
            if(result.affectedRows>0){
                return res.status(200).send({ status: 'success', message: 'phone is verified' })
            }else{
                return res.status(500).send({ status: 'error', message: 'phone is not verified' })
            }
    } catch (error) {
